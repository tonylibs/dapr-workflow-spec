...
- **HTTP and run step-runner internals** — `dws-call-http/` and `dws-run/`: the platform-level task-to-step-service contract is documented, while their individual request/response and configuration details remain deferred. The corresponding OpenAPI runner is documented in [OpenAPI step runner](integrations/openapi-step-runner.md).
