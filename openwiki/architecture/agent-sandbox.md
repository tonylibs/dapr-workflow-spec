...
The shared image is built from `agent-sandbox/Dockerfile`. It assembles the same toolchain families used by the independently built DWS components: JDK 25 for the controller and orchestrator, Go 1.26 for `dws-call-http` and `dws-run`, and Node 24 with pnpm 11.10.0 for `dws-call-openapi`. It also includes `kubectl`, `git`, `gh`, `jq`, `make`, the C toolchain needed by Go race tests, and the Claude Code, Codex, OpenSpec, OpenWiki, and ClawTeam CLIs. A Dockerfile smoke test checks those tool versions during image build.
...
`agent-sandbox/opensandbox/docker.toml` configures the OpenSandbox lifecycle server to create Docker containers through a local Docker Desktop or Docker Engine daemon. It is an alternative to the Kubernetes template, not a bridge to it: the server listens on `127.0.0.1:8080`, uses bridge networking, and persists server state in SQLite. The checked-in profile permits one intentional bind: a read-only host kubeconfig at `/root/.kube/config`; the [`start-opensandbox.ps1`](../../agent-sandbox/start-opensandbox.ps1) launcher resolves its portable placeholder into a per-process configuration before starting the server. Its Docker policy drops selected dangerous Linux capabilities, prevents privilege escalation, and limits each container to 4096 processes.
...
The same image exposes port 22 and starts `sshd-start` by default. Before starting SSH, that script adapts the read-only host kubeconfig for the local container: when Docker Desktop host-gateway DNS resolves, it copies the file to `/root/.kube-local/config`, substitutes the certificate-valid `kubernetes` hostname for `127.0.0.1` or `localhost`, and exports `KUBECONFIG` to the copy. The source bind is never rewritten. It then clones the DWS repository into an empty `/workspace`; it reuses an existing checkout only when its `origin` matches the configured repository URL, and otherwise refuses to overwrite the workspace. `DWS_REPOSITORY_URL` and `DWS_REPOSITORY_DIR` override those defaults. Finally, it generates fresh host keys for every container and starts SSH with password and keyboard-interactive authentication disabled; a caller must provide an authorized public key at `/root/.ssh/authorized_keys` and map port 22 through the Docker/OpenSandbox deployment. Keep that mapping on localhost or a protected VPN/mesh network: key injection and port mapping remain runtime-specific.

`new-ssh-sandbox.ps1` and `new_ssh_sandbox.py` automate the local SSH path using the checked-in profile settings: they ensure the dedicated `~/.ssh/dws_sandbox` key pair exists, provision its public key into the sandbox, create a localhost-only `socat` bridge on port `22222`, and wait for key-only SSH. The Python helper additionally verifies that the rewritten kubeconfig exists and that `kubectl get nodes` succeeds before creating the bridge. Both profiles request manual cleanup (no timeout), so terminate the sandbox explicitly when the session ends.
...
The image includes `kubectl` specifically for the local Docker-backed host-cluster path, but the Kubernetes session template still needs an explicit workspace-hydration design. It deliberately does not include the Dapr CLI or in-session image-building tools. Add those only when a live-cluster workflow requires them. Source: `agent-sandbox/README.md`, `agent-sandbox/sandbox.yaml`, and `agent-sandbox/cache-pvcs.yaml`.
...
- Change `opensandbox/docker.toml`, `start-opensandbox.ps1`, `sshd-start.sh`, or either SSH helper with a security review: preserve loopback/authentication requirements, the read-only kubeconfig bind and derived local copy, capability restrictions, fresh host keys, localhost-only bridge behavior, and key-only SSH unless the exposure model is deliberately redesigned.
