...
Set `dex.enabled=true` to install the chart's upstream Dex dependency. `dex.issuer` must be an issuer URL reachable by the eventual console and token validators, while `dex.consoleRedirectURI` configures the public PKCE client's redirect URI. The defaults are suitable only for in-cluster/local testing. Dex uses in-memory storage and a static password database, so this is a bootstrap/development-oriented identity-provider configuration, not a production identity lifecycle.
