...
Each orchestrator pod loads one definition once at startup from the Dapr Configuration API, using a required immutable `DEFINITION_KEY`; it does not subscribe to definition updates. `InterpreterWorkflow` runs each task list as a scope with its own program counter, so flow targets resolve only within that scope. It supports `call`, `run`, `switch`, `set`, `wait`, `listen`, `emit`, `for`, `try`, `fork`, and `raise`, including nested task lists in `try`/`catch.do`, `for.do`, and fork branches. Every supported task dispatches through a durable mechanism: `call: http`, `call: grpc`, `run: shell`, and `run: script` schedule the remote Dapr Workflow activity named `Run` against the task's step-runner app ID; `call: openapi` and `call: asyncapi` use the local activity that invokes the task app by Dapr service invocation. `call: asyncapi` then publishes an AsyncAPI `send` operation through its scoped Dapr output binding. `switch`, `set`, and `for.in`/`for.while` evaluation use local replay-safe activities; `wait` and `listen` use workflow timer/event primitives; and `emit` invokes its pub/sub activity. See `dws-orchestrator/src/main/java/io/dws/orchestrator/workflow/InterpreterWorkflow.java`, `dws-controller/src/main/java/io/dws/controller/compile/WorkflowCompiler.java`, and [OWS DSL feature roadmap](roadmap.md).
...
A matching `catch` may reference a named `use.retries` policy or declare one inline. The interpreter uses a durable timer for retry delay and re-runs the whole `try` list from the try task's original transformed input. Constant, linear, and exponential backoff, jitter, attempt-count limits, total-duration limits, and `retry.limit.attempt.duration` are supported. The per-attempt duration races the attempt's child scope against a durable timer; a deadline failure is handled by the normal catch decision path, so it counts toward retry limits and can match a timeout filter. An unhandled error, or an error in `catch.do`, propagates normally; a handled recovery completes the enclosing `try` task and then applies that task's own data-flow/output and `then` behavior.

### Timeouts and error classification

A task-level `timeout` races the task's child workflow against a durable timer; a document-level `timeout` similarly races the top-level task scope. Each can be inline or refer to `use.timeouts`. A task or retry-attempt deadline becomes the standard error `https://serverlessworkflow.io/spec/1.0.0/errors/timeout`, with status `408`, and can be selected by `catch.errors.with.type`; an instance-wide workflow deadline is outside the top-level scope and cannot be caught by a top-level `try`. Other runtime failures are classified into the same standard URI catalogue where the runtime has sufficient information (`validation`, `communication`, or `timeout`), with `runtime` retained as a 500 catch-all. See `dws-orchestrator/src/main/java/io/dws/orchestrator/error/WorkflowErrors.java` and `ErrorKind.java`.
