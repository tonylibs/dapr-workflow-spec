...
public class StackApplier {
    ...
    public StackApplier(KubernetesClient client, StackSynthesizer synthesizer,
                        EventPublisher events, DwsConfig config) {
        this.client = client;
    ...
    public ApplyResult apply(DeploymentPlan plan) {
        try {
            ...
            events.deploymentApplied(plan.workflow(), plan.versionId(), stepNames(plan), plan.orchestrator().appId());
            return new ApplyResult(plan.workflow(), plan.versionId(), plan.version(), !alreadyDeployed);
        } catch (RuntimeException e) {
            events.deploymentFailed(plan.workflow(), plan.versionId(), stepNames(plan),
                    plan.orchestrator().appId(), e.getMessage());
            throw e;
        }
    }
    ...
    private void markForDrain(Deployment deployment) {
        ...
        client.apps()
                ...
                .edit(current -> new DeploymentBuilder(current)
                        ...
                        .build());
        String workflow = deployment.getMetadata().getLabels().get(Labels.WORKFLOW);
        events.deploymentDrained(workflow, versionOf(deployment), orchestratorAppId(deployment, workflow));
    }
