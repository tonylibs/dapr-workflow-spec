...
`dws-controller`'s obligation to map each DSL `run` subtype to the correct step image and
`TaskKind`, to reject unsupported subtypes at compile time rather than at deploy time, and
`dws-orchestrator`'s obligation to dispatch `run` tasks over the existing service-invocation
path. Established in `dws-run`.
