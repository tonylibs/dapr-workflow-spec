...
The `charts/dws` Helm chart's rendering of the `dws-controller` control plane — ServiceAccount,
RBAC, Deployment, and Service — as templatized, values-driven resources equivalent to the
reference manifests in `dws-controller/k8s/`. Established in `helm-controller-templates`
(Helm packaging roadmap Phase 2).
