...
`dws-controller`'s publishing of definition, deployment, and garbage-collection lifecycle events to
the shared `dws.events` topic, as a fire-and-forget side effect of the apply pass. See
`docs/events.md` for the shared envelope and event catalog, and `lifecycle-events` for the
