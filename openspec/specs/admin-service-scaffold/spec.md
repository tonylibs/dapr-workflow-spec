...
The `dws-admin` NestJS project itself — module layout (`ConfigModule`/`StoreModule`/
`DaprEventsModule`/`WorkflowsModule`/`InstancesModule`), health endpoint, and local dev setup
(docker-compose + `dapr run`). Established in `dws-admin-skeleton` (Epic 2).
