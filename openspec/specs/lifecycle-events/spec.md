...
The cross-component contract for DWS lifecycle events: the shared topic and pub/sub component, the
CloudEvents-style envelope, and every `io.dws.*` event type with its payload shape. Independently
implemented by `dws-controller` (see `controller-event-publishing`) and `dws-orchestrator` (see
`orchestrator-event-publishing`), and documented at the repository root in `docs/events.md` because
both components rely on the same contract.
