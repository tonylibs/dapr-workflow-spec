# asyncapi-step-compilation

## Purpose

`dws-controller`'s compile-time handling of `call: asyncapi` tasks: fetching and content-pinning the
AsyncAPI document, a light read of the server protocol/host and the operation's channel address,
selecting a Dapr output-binding component type from the protocol, synthesizing a version-scoped Dapr
binding `Component` with secret-backed credentials, and pinning the runner's environment. Established
in `dws-call-asyncapi` (OWS DSL roadmap Phase 5, AsyncAPI slice).
