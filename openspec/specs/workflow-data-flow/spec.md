...
`dws-orchestrator`'s per-task data-flow pipeline: `input.from`/`output.as`/`export.as`
transformation via jq, `input.schema`/`output.schema` JSON Schema validation, the persistent
workflow context document that `export.as` writes and expressions read as `$context`, and the
minimal validation/transform fault shape. Established in `data-flow-pipeline` (OWS DSL roadmap
Phase 1).
