...
The environment-variable contract between `dws-controller` and the `dws-run` step images —
`MODE`, `TASK`, `PORT`, `COMMAND`, `SCRIPT`, `ARGUMENTS`, `ENVIRONMENT`, `RETURN`, `OUTPUT`,
`TIMEOUT` — including how a DSL `arguments` map renders per runtime and how one Go codebase
produces three images. Established in `dws-run`.
