...
`dws-orchestrator`'s `fork` task: running `fork.branches` concurrently, joining (`compete: false`)
or racing (`compete: true`) their outcomes, per-branch data/`$context` isolation, name resolution
across nested scopes, and composition with `try`/`catch`/retry. Established in `fork-task` (OWS DSL
roadmap Phase 2, slice 2.4).
