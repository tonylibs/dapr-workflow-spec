# activity-step-worker

## Purpose

How the Go step images (`dws-call-http` and the three `dws-run` images) run as Dapr Workflow
activity workers: each registers a single canonical `Run` activity against its app-id, wrapping
the existing `runner.Run`, and preserves the prior HTTP behavior for input, empty results,
`OUTPUT` shaping, and the retryable-vs-configuration failure distinction.
