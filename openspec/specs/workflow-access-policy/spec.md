...
How `dws-controller` secures cross-app activity dispatch: it synthesizes one Dapr
`WorkflowAccessPolicy` per activity-invoked step, allowing the workflow's orchestrator app-id to
schedule the canonical `Run` activity on that step. `call: openapi` steps get none. Policies are
labeled and garbage-collected with the rest of the workflow's stack.
