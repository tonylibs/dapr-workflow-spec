...
`dws-admin`'s read-only REST surface over the read model — the workflow and instance list/detail
endpoints, their query filters and pagination, the request/response DTO contract, and the generated
`@nestjs/swagger` OpenAPI document a typed client (`dws-console`) is generated from. Established in
`dws-admin-read-api` (Epic 3).
