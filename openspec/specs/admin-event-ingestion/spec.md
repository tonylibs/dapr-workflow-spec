...
`io.dws.task.completed`, `io.dws.task.failed`. `dws-admin` SHALL maintain exactly one Dapr
subscription per `(pubsubName, topic)` pair — the Dapr SDK rejects a second subscription to the
same pair — so all event types funnel through a single subscription entry point that dispatches
internally by `type`.
