# admin-read-model-schema

## Purpose

The Postgres/Drizzle schema for `dws-admin`'s read model (`workflow_definitions`, `deployments`,
`workflow_instances`, `task_events`, `processed_events`) and its migration discipline (checked-in
`drizzle-kit generate` output only). Established in `dws-admin-skeleton` (Epic 2).
...
an indexed `instance_id`, `task_name`, `type`, `status`, `timestamp`, `error`. `task_events.instance_id`
SHALL NOT be a database-enforced foreign key into `workflow_instances`: under at-least-once,
unordered Dapr pub/sub delivery, a `task.*` event can be processed before the corresponding
`instance.*` event, and a hard foreign key would reject that insert.

#### Scenario: task_events eventually corresponds to a known instance
- **WHEN** a `task_events` row is inserted for a given `instance_id`
- **THEN** that `instance_id` corresponds to a row in `workflow_instances` once the corresponding
  `instance.*` event has been processed, regardless of which arrived first

#### Scenario: A task event for an as-yet-unknown instance is not rejected
- **WHEN** a `task.*` event for an `instance_id` is processed before any `instance.*` event for
  that `instance_id` has been processed
- **THEN** the `task_events` insert succeeds
