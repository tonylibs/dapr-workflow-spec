...
- [x] 4.4 Verified — pushed to PR #43; `verify`, `integration`, and `integration-dapr-preinstalled` all pass after several rounds of CI-driven fixes (Chart.lock/dependency fetch, dry-run CRDs, sidecar-injector timing, the `dapr.enabled` scope correction above).
