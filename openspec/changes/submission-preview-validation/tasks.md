...
- [ ] 1.1 Add `scripts/vendor-dsl-schema.mjs` to `dws-admin`: read
...
- [ ] 1.2 Have the script write `src/definition-validation/schema/workflow-schema.json`
...
- [ ] 1.3 Run the script and commit both generated files. Confirm the schema `$id`
  is the `1.0.1` identifier and record it in provenance.
- [ ] 1.4 Add `schema-provenance.spec.ts`: assert provenance `sdkVersion` matches
...
- [ ] 2.1 Add deps `ajv`, `ajv-formats`, `yaml` to `dws-admin`.
- [ ] 2.2 Write failing tests for `DefinitionValidationService.validate`: valid
...
- [ ] 2.3 Implement the service: parse with `yaml`, then validate with `Ajv2020`
  (`allErrors: true`, `strict: false`, `ajv-formats`), compiling the schema once at
  construction. Map ajv errors to `{path, message, keyword}`.
- [ ] 2.4 Add the 50-error cap and `truncated` flag, with a test that exceeds it.
- [ ] 2.5 Write failing tests for nested task-name uniqueness (duplicate across a
...
- [ ] 2.6 Add a fixture-parity test: every definition under
...
- [ ] 3.1 Write failing controller tests: 200 `{valid:true}`; 200 `{valid:false,
  errors}`; 400 on empty body; 400 on unsupported content type; 413 above 1 MiB.
- [ ] 3.2 Implement `DefinitionValidationController` (`POST /definitions/validate`)
...
- [ ] 3.3 Confirm `main.ts`'s existing raw parser covers the accepted YAML content
  types; extend only if a gap shows up in the tests.
- [ ] 3.4 Run `pnpm lint && pnpm test && pnpm build` in `dws-admin`; commit.

## 4. dws-console transport

- [ ] 4.1 Write failing tests in `admin-client.test.ts` for
  `validateDefinitionSpec`: sends the raw buffer with the format's content type,
  returns the report, throws `ApiError` on non-2xx.
- [ ] 4.2 Implement `validateDefinitionSpec` in `admin-client.ts`.
- [ ] 4.3 Write failing tests for `previewDefinition`: posts `?dryRun=true`,
  returns `{kind:"plan"}` on success, `{kind:"deploy-error"}` on 400 with
  `errors[]`, throws `ApiError` on an unexpected plan shape.
- [ ] 4.4 Add the zod `deploymentPlanSchema` and implement `previewDefinition`,
  leaving `submitDefinition` byte-identical.

## 5. dws-console editor UI

- [ ] 5.1 Write failing component tests for `/workflows/new`: preview disabled when
...
- [ ] 5.2 Add the preview control and the sequential two-layer handler to
  `src/routes/workflows/new.tsx`, leaving submit untouched.
- [ ] 5.3 Add plan rendering (version header, steps table, bindings table,
  orchestrator) using existing console components and tokens.
- [ ] 5.4 Clear a rendered preview when the buffer changes, so a stale plan is
  never read as current.
- [ ] 5.5 Run `pnpm lint && pnpm test && pnpm build` in `dws-console`; commit.

## 6. Documentation

- [ ] 6.1 Update `docs/roadmaps/dws-console-submission.md`: Phase 2 row to done,
...
- [ ] 6.2 Note in §6 that duplicate task names are enforced in both layers, and
  that `dws-admin`'s check is early/path-precise parity rather than new coverage.
- [ ] 6.3 Update the Notion mirror
  (`https://app.notion.com/p/3c92f73e4fd981988252dcbff0736f60`) to match.
- [ ] 6.4 Update `dws-admin`'s README with the new endpoint and the
