...
- [ ] 1.1 Write `openspec/schemas/single-node-definition.schema.json`: a `kind`-discriminated
...
- [ ] 1.2 Add `flow` branch: `required: [scope, tasks, children]`, `scope` enum
  (`main`/`for`/`try`/`catch`/`forkBranch`), `catch` optional string.
- [ ] 1.3 Add `step` branch: `required: [task]`, `functionAppId` optional string (schema can't
...
- [ ] 1.4 Add two or three hand-written sample definition files under
...
- [ ] 1.5 Validate the schema itself and all three examples with a JSON Schema validator (e.g.
...
- [ ] 2.1 Create `dws-step/` mirroring `dws-orchestrator`'s Maven layout: `pom.xml` (Java 25,
...
- [ ] 2.2 Add a definition loader (`SingleNodeDefinitionLoader` or similar): reads the file path
...
- [ ] 2.3 Register one Dapr Workflow Activity named `Step` (constant name, per design D3) whose
  body logs the loaded task's kind and returns immediately — no real dispatch logic yet.
- [ ] 2.4 Add `GET /healthz` (a plain `@RestController`, not Spring Boot Actuator's default path)
  reporting healthy once the definition loader has completed successfully at boot.
- [ ] 2.5 Add a `Dockerfile` matching `dws-orchestrator`'s existing multi-stage Maven build shape.
- [ ] 2.6 Write `dws-step/README.md`: local dev section with the `dapr run` invocation (app-id,
...
- [ ] 2.7 Add unit tests for the definition loader (valid flow-shape file rejected; valid step file
...
- [ ] 2.8 Validation: `cd dws-step && ./mvnw verify` — **could not be run in this environment**
  (only OpenJDK 11 and no standalone Maven are installed here; the project targets Java 25 via the
  wrapper). Flag as unverified in the PR description; must be confirmed once CI or a Java-25-capable
  machine is available.

## 3. dws-flow scaffold (.NET, Dapr Workflow SDK)

- [ ] 3.1 Create `dws-flow/` as a new .NET project (`dws-flow.csproj`, worker-service style
  `Program.cs`), with `Dapr.Workflow` and `Dapr.Client` package references.
- [ ] 3.2 Add a definition loader mirroring 2.2's shape and validation rules, for the `flow`
...
- [ ] 3.3 Register one Dapr Workflow type named `Flow` (constant name, per design D2) whose body
  logs the loaded scope and task count and returns immediately.
- [ ] 3.4 Add `GET /healthz` reporting healthy once the definition loader has completed
  successfully at boot.
- [ ] 3.5 Add a `Dockerfile` (multi-stage `dotnet publish` build), following the shape of the
...
- [ ] 3.6 Write `dws-flow/README.md`: local dev section with the `dapr run` invocation against one
  of task 1.4's sample files, same shape as 2.6.
- [ ] 3.7 Add unit tests for the definition loader (same coverage as 2.7, for the `flow` shape).
- [ ] 3.8 Validation: `cd dws-flow && dotnet build` — **could not be run in this environment** (no
  .NET SDK installed here). Flag as unverified; must be confirmed once CI or a .NET-capable machine
  is available.
