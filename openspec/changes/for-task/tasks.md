...
- [ ] 1.1 Add `EvaluateForRequest(String taskName, JsonNode data, Map<String, JsonNode>
      variables)` implementing `StepRequest` (mirroring `EvaluateSwitchRequest`/
      `EvaluateSetRequest`/`RaiseErrorRequest` shape)
- [ ] 1.2 Add `EvaluateForActivity` (in-process, no I/O — parallel to `EvaluateSwitchActivity`)
      ...
      `JsonNode`
- [ ] 1.3 Reject a non-array result with `IllegalStateException` carrying the task name in the
      message so replay stays deterministic and the failure reaches the standard task-failure
      path
- [ ] 1.4 Register `EvaluateForActivity` in `WorkflowRuntimeBootstrap`
- [ ] 1.5 Unit tests in `EvaluateForActivityTest`: bare-jq `.pets` expression, `${...}`-wrapped
...
- [ ] 2.1 Add `EvaluateWhileRequest(String taskName, JsonNode data, Map<String, JsonNode>
      variables)` implementing `StepRequest`
- [ ] 2.2 Add `EvaluateWhileActivity` (in-process, no I/O — parallel to `EvaluateSwitchActivity`)
...
- [ ] 2.3 Register `EvaluateWhileActivity` in `WorkflowRuntimeBootstrap`
- [ ] 2.4 Unit tests in `EvaluateWhileActivityTest`: truthy jq result (`.count > 0`), falsy
...
- [ ] 3.1 Add an `else if (item.getTask().getForTask() != null)` branch to
      `DefinitionLookup.search` mirroring the existing `TryTask` branch — recurse into
      `forTask.getDo()`, returning the found `Task` if any
- [ ] 3.2 Unit test in `DefinitionLookupTest` (create if absent, mirroring existing
...
- [ ] 4.1 Replace the `case ForTask _ -> throw new UnsupportedOperationException(...)` stub in
      `dispatchConcreteTask` with `case ForTask forTask -> dispatchFor(ctx, forTask, name, data,
      context, variables, depth, events, mapper);`
- [ ] 4.2 Add a private `dispatchFor` helper mirroring `dispatchTry`'s method shape. It calls
...
- [ ] 4.3 For each element in the collection: clone `variables` into a new `HashMap`, bind
...
- [ ] 4.4 When `forTask.getWhile()` is non-null and non-blank, call `EvaluateWhileActivity` via
...
- [ ] 4.5 Call `runTaskList(ctx, forTask.getDo(), data, context, scoped, depth + 1, events,
...
- [ ] 4.6 After the loop, return `new Body(data, context, FlowOutcome.of(forTask.getThen()),
      ScopeEnd.FELL_THROUGH)` with the final iteration's data
- [ ] 4.7 Confirm no change is needed to `taskTypeOf` — the existing `for` branch (lines
      482–483) is already correct

## 5. Integration tests in `InterpreterWorkflowIntegrationTest`

- [ ] 5.1 Add a stub for `EvaluateForActivity` and `EvaluateWhileActivity` in the test's
...
- [ ] 5.2 Basic iteration case: a `for` over a three-element array, `for.do` is a `set` task
...
- [ ] 5.3 Index-variable case: a `for` whose body records `$index` per iteration; assert the
      recorded indices are `[0, 1, 2, ...]`
- [ ] 5.4 `while` early-exit case: a `for` over a five-element array whose `while` becomes false
      at index 2; assert only two iterations run and the final data reflects that
- [ ] 5.5 Empty-collection case: a `for` whose `for.in` evaluates to `[]`; assert the body runs
      zero times, the incoming data flows through unchanged, and the `for` task's own
      lifecycle event pair still fires exactly once
- [ ] 5.6 `for` nested inside a `try` case: a task inside `for.do` raises an error caught by
      the enclosing `try`'s `catch.errors.with`; assert the failure is caught and the workflow
      continues past the `try` task
- [ ] 5.7 Non-array `for.in` case: a `for` whose `for.in` evaluates to a scalar; assert the
      task fails with a message naming the task and the standard instance-failure path fires

## 6. Verification and roadmap update

- [ ] 6.1 Run `./mvnw verify` in `dws-orchestrator/`; confirm green
- [ ] 6.2 Run `./mvnw test` in `dws-controller/`; confirm green and that a definition
      ...
      made in this slice)
- [ ] 6.3 Update `docs/roadmaps/openworkflow-features.md` §1 (`for` row: `⚠️` → `✅`, description
