...
category as `switch`, `set`, `wait`, and `raise`. Owning component: `dws-controller`.

#### Scenario: Definitions with a `for` task compile unchanged
- **WHEN** a definition declares a `for` task whose `for.do` contains only tasks that themselves
  ...
  `try` whose contents likewise deploy nothing)
