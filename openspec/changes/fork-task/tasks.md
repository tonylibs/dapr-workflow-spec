...
- [x] 1.1 Add an `else if (task.getForkTask() != null)` branch to `DefinitionLookup.search`
      mirroring the existing `TryTask`/`ForTask` branches — recurse into
      `forkTask.getFork().getBranches()`, returning the found `Task` if any
- [x] 1.2 Unit test in `DefinitionLookupTest` (mirroring existing per-unit test style): a task name
...
- [x] 2.1 Add an `else if (task.getForkTask() != null)` branch to `WorkflowCompiler.walk()`
...
- [x] 2.2 Add an `else if (task.getForTask() != null)` branch to `WorkflowCompiler.walk()` —
      recurse into `task.getForTask().getDo()`, closing `for-task`'s logged gap
- [x] 2.3 Update the stale comment (`// switch/set/wait/for/raise (and the task lists nested under
      for/fork) deploy nothing.`) to reflect that `for`/`fork` bodies are now walked
- [x] 2.4 Add the same two branches (`ForkTask`, `ForTask`) to `collectTaskNames()` so a duplicate
      task name inside a `fork` branch or `for.do` is rejected at compile time
- [x] 2.5 Unit tests in `WorkflowCompilerTest`: a `call` task inside a `fork` branch compiles to a
...
- [x] 3.1 Add `ForkBranchInput(String taskName, JsonNode data, JsonNode context, Map<String,
      JsonNode> variables, int depth)` (record, Jackson-serializable — same shape discipline as
      `EvaluateForRequest`/`RaiseErrorRequest`)
- [x] 3.2 Widen `InterpreterWorkflow.dispatch(...)` from `private` to package-private (same pattern
      `runTaskList` already uses) so `ForkBranchWorkflow` can call it directly
- [x] 3.3 Add `ForkBranchWorkflow implements Workflow` with a fixed registration name constant
...
- [x] 3.4 Confirm `AdminEventBuilder.forContext(ctx)` works correctly when `ctx` is a child
...
- [x] 4.1 Add `ForkTask` to `dispatchBody`'s `StreamEx.of(...)` list (alongside `getForTask()`,
      `getTryTask()`, `getRaiseTask()`, etc.) so a `fork` task reaches `dispatchConcreteTask`
- [x] 4.2 Add `case ForkTask forkTask -> dispatchFork(ctx, forkTask, name, data, context,
      variables, depth, events, mapper);` to `dispatchConcreteTask`'s switch
- [x] 4.3 Add a private `dispatchFork` helper mirroring `dispatchTry`/`dispatchFor`'s method shape.
      ...
      declared order
- [x] 4.4 `compete: false` (or `fork.isCompete() == false`): `ctx.allOf(handles).await()` →
...
- [x] 4.5 `compete: true`: build a `List<Task<?>>` wildcard copy of the handles, call
      ...
      never awaited
- [x] 4.6 Add a `fork` branch to `taskTypeOf` (`task.getForkTask() != null` → `"fork"`)

## 5. Bootstrap registration

- [x] 5.1 Register `ForkBranchWorkflow` in `WorkflowRuntimeBootstrap.startRuntime()`
...
- [x] 6.1 Add whatever stub/support the test harness needs for `ctx.callChildWorkflow`/`allOf`/
...
- [x] 6.2 `compete: false` join case: a `fork` task with three branches, each a `set` task
...
- [x] 6.3 `compete: true` race case: a `fork` task with two branches; assert the `fork` task
      completes with the first-settled branch's data and the workflow does not block on the other
- [x] 6.4 Branch failure case (`compete: false`): one branch's task fails; assert the `fork` task
      fails with `CompositeTaskFailedException`-shaped detail and, when nested in a `try`, is
      caught by `catch.errors.with`
- [x] 6.5 Winning-branch failure case (`compete: true`): the first-settled branch fails; assert the
      failure surfaces as the `fork` task's own failure
- [x] 6.6 Nested-container branch case: a branch whose task is a `try` containing multiple steps
      with a retry; assert the retry runs correctly inside the branch
- [x] 6.7 `$context` isolation case: a branch performs `export.as`; assert neither a sibling branch
      nor the task after `fork` observes that write
- [x] 6.8 `fork` nested inside a `try` case: a failure inside a branch is caught by the enclosing
      `try`'s `catch.errors.with`; assert the workflow continues past the `try` task

## 7. Verification and roadmap update

- [x] 7.1 Run `./mvnw verify` in `dws-orchestrator/`; confirm green
- [x] 7.2 Run `./mvnw test` in `dws-controller/`; confirm green and that a `fork`/`for` task
...
- [x] 7.3 Update `docs/roadmaps/openworkflow-features.md` §1 (`fork` row: `❌` → `✅`; "nested `do`"
