...
- [ ] 2.2 Add `redis` as a conditional Bitnami subchart dependency (`condition: redis.enabled`)
      to `charts/dws/Chart.yaml`, pinning a version consistent with the other Bitnami dependency
      (`postgresql`); run `helm dependency update charts/dws` to refresh `Chart.lock`.
- [ ] 2.3 Add a `redis:` block to `charts/dws/values.yaml`: `enabled: true`,
      `architecture: standalone`, `auth.password`, `master.persistence.size` (or the subchart's
      equivalent key), `networkPolicy.enabled: false`, and — if 2.1 shows it's needed —
      `image.repository: bitnamilegacy/redis` next to the existing
      `global.security.allowInsecureImages` comment. Add
      `redis.external.{host,existingSecret,existingSecretKey}` values for the disabled-Redis path.
- [ ] 2.4 Add `dws.redis.host`, `dws.redis.secretName`, and `dws.redis.secretKey` helpers to
      ...
      when `redis.enabled=true` and to `redis.external.*` when `false` (per design.md).
- [ ] 2.5 `helm lint charts/dws` and `helm template charts/dws` (default values, then
      `--set redis.enabled=false --set redis.external.host=... --set
      redis.external.existingSecret=...`); confirm the Bitnami Redis subchart resources render
      only in the first case.

## 3. Dapr Component templates

- [ ] 3.1 Add `charts/dws/templates/pubsub-component.yaml` — `dapr.io/v1alpha1` `Component`,
      type `pubsub.redis`, name `pubsub`, gated by `.Values.dapr.enabled` and Redis
      availability, using the `dws.redis.*` helpers for `redisHost`/`redisPassword`
      (`secretKeyRef`), `enableTLS: "false"`, `auth.secretStore: kubernetes` — shaped like
...
- [ ] 3.4 `helm template charts/dws` with default values; confirm all three Components render
      ...
      `--set dapr.enabled=false` and with `--set redis.enabled=false` (no external Redis set);
      confirm none of the three render in either case.
...
- [ ] 5.2 `helm template charts/dws` — default values, `dapr.enabled=false`,
      `redis.enabled=false` (with external Redis set), and `controller.enabled=false` — confirm
      no unexpected resources render or disappear outside what each toggle should affect.
