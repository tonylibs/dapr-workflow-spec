 ...
 *
 * <p>By convention a {@code call} task named {@code checkInventory} invokes the Dapr app-id {@code
 * check-inventory}; the same kebab-case mapping is used for {@code emit} topics. This is the only
 * orchestrator-specific interpretation layered on top of the SDK model.
 */
public final class TaskNaming {
  ...
  public static String toKebabCase(String taskName) {
    ...
    String withHyphens =
        taskName.replaceAll("([a-z0-9])([A-Z])", "$1-$2").replace('_', '-').replace(' ', '-');
