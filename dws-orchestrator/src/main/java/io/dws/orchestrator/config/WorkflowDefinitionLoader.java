 ...
 * still be starting), then parsed and structurally validated with the Open Workflow Specification
 * SDK. Any missing key or invalid document raises {@link DefinitionLoadException}; the bootstrap
 * turns that into a clear log line and a non-zero exit. Configuration changes are deliberately not
...
public class WorkflowDefinitionLoader {
  ...
  public WorkflowDefinitionLoader(
      DaprClient client, String store, String key, int maxAttempts, Duration retryInterval) {
    this.client = client;
    this.store = store;
    this.key = key;
  ...
  public Workflow load() {
    ...
    LOG.info(
        "Loaded workflow '{}' version '{}' with {} top-level task(s)",
  ...
  private String fetchWithRetry() {
    ...
    for (int attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        lastError = e;
        LOG.warn(
            "Configuration fetch failed (attempt {}/{}): {}", attempt, maxAttempts, e.getMessage());
      }
  ...
  private void sleepBeforeRetry(int attempt) {
    ...
    try {
      ...
      throw new DefinitionLoadException(
          "interrupted while waiting to retry configuration fetch", e);
    }
  }

  private Workflow parse(String text) {
    WorkflowFormat format =
        text.stripLeading().startsWith("{") ? WorkflowFormat.JSON : WorkflowFormat.YAML;
    try {
      return WorkflowReader.readWorkflowFromString(text, format);
    } catch (Exception e) {
      throw new DefinitionLoadException(
          "failed to parse workflow definition: " + e.getMessage(), e);
    }
  }
  ...
  private void validateStructure(Workflow workflow) {
    ...
    if (workflow.getDo() == null || workflow.getDo().isEmpty()) {
      throw new DefinitionLoadException(
          "workflow '" + workflow.getDocument().getName() + "' has no 'do' tasks");
    }
  }
