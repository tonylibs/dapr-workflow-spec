...
| `dws-console` | Yes, optional | Deployment/Service/Ingress rendered against `ghcr.io/tonylibs/dws-console`, gated behind `console.enabled` (default `false`). See [`dws-console` roadmap](dws-console.md) |
...
| 6. Console | ✅ | Add Deployment/Service/Ingress once an image exists | `templates/console/{deployment,service,ingress}.yaml` render behind `console.enabled` (default `false`) against the `ghcr.io/tonylibs/dws-console` image on container port 3000, health path `/healthz`. Skips Dapr sidecar annotations and the postgres/redis init-container waits by design — the console talks to `dws-admin`/`dws-controller` browser-direct over CORS (see [`dws-console` roadmap §5](dws-console.md#5-open-items)). `dws.console.fullname`/`dws.console.selectorLabels` added to `_helpers.tpl`. Ingress is chart-first: single-rule (host/path/className/annotations/tls) routing the configured host to the console Service, `console.ingress.enabled` off by default; does **not** front `dws-admin` (see the follow-up open item below). |
| 7. Values design | ✅ | Finalize `values.yaml` | `defaults.{resources,nodeSelector,tolerations,affinity}` now consistently configures every chart-owned Deployment (controller, admin, admin-gateway, console), with deep-merged per-component overrides and a rendered CI assertion. PostgreSQL retains the matching upstream `postgresql.primary.*` StatefulSet schema. Controller deliberately has no Ingress because it is outbound-only; console/admin routing deliberately remains operator-owned until the `dws-console` runtime-config shim resolves its per-install admin URL. |
...
- **Next up:** Phase 10 (docs — README/CLAUDE.md `helm install` reference), Phase 11 (Knative — independent, on its own timeline), or the two console follow-ups called out above (runtime-config shim for `VITE_DWS_ADMIN_URL`, unified console-plus-admin ingress). Phases 5 and 6 completed; the actor-statestore Component ships ready but unused until `dws-orchestrator` adopts the Dapr Workflow runtime (tracked separately, not on this roadmap).
