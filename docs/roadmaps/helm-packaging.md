...
| 7. Values design | ⚠️ | Finalize `values.yaml` | Controller/admin/postgresql/console image/tag/resources/replicas + console ingress + global namespace done; still no ingress convention for controller/admin (open question), and no shared `resources`/`nodeSelector`/`tolerations`/`affinity` schema across components. |
