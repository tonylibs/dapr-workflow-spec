...
| 5. Event wiring | ⚠️ | Wire the controller→admin pub/sub path chart-side | Admin Deployment already carries `dapr.io/enabled`/`dapr.io/app-id` annotations + `DAPR_PUBSUB_NAME`/`DAPR_PUBSUB_TOPIC` env vars (hardcoded, not gated behind a toggle since Phase 4 doesn't exist yet). Still missing: controller-side dapr annotations, a `pubsub-component.yaml` Dapr Component template, and an end-to-end test that asserts a submitted definition actually lands in admin's read model via pub/sub (today's `helm test` only checks DB connectivity). Depends only on Phase 4 (Dapr) — not on Phase 11 (Knative) |
