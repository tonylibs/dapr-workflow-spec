...
| [Observability](observability.md) | Cross-cutting — OpenTelemetry metrics, traces, and logs from every DWS component, chart-managed and controller-stamped alike | ❌ nothing started. Phase 0 spikes first: whether Knative Serving allows the OTel Operator's init-container injection (blocks the whole mechanism for step services), OTel Operator as dependency vs. prerequisite, and trace-context behaviour across Dapr Workflow replay. Phase 3 (controller stamps injection annotations onto per-workflow resources) is the gate — before it, only `dws-controller`/`dws-admin` are covered |
