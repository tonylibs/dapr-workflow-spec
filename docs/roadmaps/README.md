...
| [Helm chart packaging](helm-packaging.md) | `charts/dws` — cluster install of the control plane | Phases 0–4 done (controller, admin+DB with in-chart Postgres, Dapr as a conditional chart dependency + preflight check + sidecar self-heal hook) plus Phases 8–9 done (full lint/template/kind-integration CI, OCI publish to ghcr.io). Phase 5 (event wiring) partial — admin has Dapr sidecar annotations but no pubsub Component yet. **Phase 5 (controller-side dapr annotations, `pubsub-component.yaml`, end-to-end pub/sub test) next**, now unblocked by Phase 4; Phase 6 (console) still blocked, Phase 10 (docs) not started, Phase 11 (Knative — split out of Phase 4) deferred and independent |
