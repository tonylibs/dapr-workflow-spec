...
| [Observability](observability.md) | Cross-cutting — OpenTelemetry metrics, traces, and logs from every DWS component, chart-managed and controller-stamped alike | Restructured 2026-09-19 into **Track A** (ordinary Deployments — start now) and **Track B** (Knative step services — deferred). Phase 0-A closed: OTel Operator is a documented prerequisite + preflight, `dws-flow` is glibc so three annotations are the complete set, operator pinned to chart 0.123.0 / 0.159.0; the Dapr Workflow replay question is deferred to observation during testing. **Phase 1 (chart surface — `dws-controller` + `dws-admin`) landed 2026-09-19 and is ⚠️, not ✅**: the `observability.*` values block, `Instrumentation` CR, merged per-component Dapr tracing (`dapr.io/config` is single-valued, so tracing shares the Configuration auth already owns), operator preflight, targeted Java/Node.js injection, and a four-way auth × observability render test with a byte-identical disabled-state baseline are all in CI — what remains is the live controller → Dapr → admin → Postgres trace, which needs a cluster with the pinned Operator and a reachable collector. Phase 2a covers `dws-orchestrator` only; `dws-flow`/`dws-step` wait on [workflow-runtime-architecture-roadmap.md](workflow-runtime-architecture-roadmap.md) Phase 4, which nothing deploys them without. Track B waits on the Knative init-container spike, now scoped to just `dws-call-openapi`/`-asyncapi`/`-a2a` |
