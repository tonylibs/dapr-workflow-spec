...
flowchart TD
  ...
  P25 --> P5["Phase 5: Auth ⚠️<br/>OIDC login implemented;<br/>Dex 2.44 session/logout blocker"]
...
| **5** | Console-level auth (login, session, RBAC on write actions) | [OWS Phase 4 — auth/secrets](openworkflow-features.md) for backend parity | ⚠️ partial — browser OIDC/PKCE login is implemented and locally gated. Live Docker Desktop validation fixed Dex CORS, then found Dex 2.44.0 lacks compatible `prompt=none` sessions and RP logout; the Dapr-gated write path also remains. See [`dws-auth.md`](dws-auth.md) |
...
| Auth (Phase 5) | `components/auth-control.tsx` (+ `auth-control.test.tsx`), `lib/oidc.ts`, `lib/oidc-config.ts` (+ `oidc-config.test.ts`), `vite.config.ts`, `.env.example` | ⚠️ partial. OIDC Authorization Code + PKCE bootstrap, app-wide in-memory auth state, sign-in/identity/logout UI, SSR wiring, and redirect-URI config are merged. A live chart-installed Dex run verified discovery/CORS/S256 and exposed the remaining Dex 2.44.0 session/RP-logout blocker; Phases 2–5 of the dedicated [`dws-auth.md`](dws-auth.md) write-path sequence also remain. |
...
the browser OIDC login foundation, while its live run is blocked by missing Dex 2.44.0 browser
session/RP-logout support and its Dapr-gated write path remains ([`dws-auth.md`](dws-auth.md)).
