...
// upstream-vs-config failure distinction the shared step contract carries.
package activity
...
// orchestrator reads. Upstream (gRPC non-OK status) and transport faults become
// the retryable "upstream failure" marker; all other errors (config, descriptor,
// encode, decode) become the non-retryable "config failure" marker.
