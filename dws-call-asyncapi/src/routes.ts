/**
 * HTTP surface for the step: POST /run and GET /healthz. /run dispatches the
 * operation through the local Dapr output binding and shapes the response per
 * OUTPUT; failures map to the platform error contract (400 for bad input, 502
 * for upstream/transport, 500 otherwise). /healthz reports readiness — it is
 * only reachable once all plugins (and thus full initialization) have completed.
 */
